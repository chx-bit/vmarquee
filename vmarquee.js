...
(function (global) {
  ...
  function injectStyles() {
    ...
    style.textContent = [
      '.vmarquee__track{display:flex;width:max-content;will-change:transform;flex-shrink:0;}',
      '.vmarquee[data-direction="up"] .vmarquee__track,',
      '.vmarquee[data-direction="down"] .vmarquee__track{flex-direction:column;width:auto;height:max-content;}',
      '.vmarquee__item{flex-shrink:0;display:inline-block;}',
  ...
  class VMarquee {
    constructor(el) {
      ...
      this.track     = null;
      this.paused    = false;
      this._bound    = { pause: this.pause.bind(this), play: this.play.bind(this) };
      this._resizeOb = null;
      this.options   = this._parseOptions();

      if (!ANIMATION_MAP[this.options.direction]) {
        console.warn('[vMarquee] Unknown direction "' + this.options.direction + '". Falling back to "left".');
    ...
    _parseOptions() {
      ...
      return {
        ...
        speed:        Math.max(1, parseFloat(d.speed))  || DEFAULTS.speed,
        gap:          Math.max(0, parseFloat(d.gap))    || DEFAULTS.gap,
    ...
    _build() {
      ...
      if (!originalContent) {
        console.warn('[vMarquee] Element has no content. Skipping:', this.el);
        return;
      }
    ...
    _measure(firstItem, isVertical, speed, direction, attempt) {
      attempt = attempt || 0;
      const itemSize      = isVertical ? firstItem.offsetHeight : firstItem.offsetWidth;
      const containerSize = isVertical ? this.el.offsetHeight   : this.el.offsetWidth;

      if (itemSize === 0 || containerSize === 0) {
        if (attempt < 10) {
          requestAnimationFrame(() => this._measure(firstItem, isVertical, speed, direction, attempt + 1));
        } else {
          console.warn('[vMarquee] Could not measure element after 10 attempts. Element may be hidden:', this.el);
        }
        return;
      }

      const existing = this.track.querySelectorAll('.vmarquee__item');
      existing.forEach((el, i) => { if (i > 0) el.remove(); });

      const needed   = Math.ceil((containerSize * 3) / itemSize) + 2;
      const fragment = document.createDocumentFragment();
      for (let i = 1; i < needed; i++) {
        fragment.appendChild(firstItem.cloneNode(true));
      }
      this.track.appendChild(fragment);

      this.track.style.setProperty('--vm-offset', '-' + itemSize + 'px');
      this.track.style.animation = ANIMATION_MAP[direction] + ' ' + speed + 'ms linear infinite';
    ...
    _watchResize(firstItem, isVertical, speed, direction) {
    ...
    _attachEvents() {
      ...
      this.el.addEventListener('mouseenter', this._bound.pause);
      this.el.addEventListener('mouseleave', this._bound.play);
    }
    ...
    destroy() {
      if (this.options.pauseOnHover) {
        this.el.removeEventListener('mouseenter', this._bound.pause);
        this.el.removeEventListener('mouseleave', this._bound.play);
      }
      if (this._resizeOb) {
  ...
  function init(selector) {
    ...
    els.forEach(el => {
      ...
      try {
        ...
        console.error('[vMarquee] Failed to init element:', el, err);
      }
    });
    return instances;
  }
  ...
  function getInstance(el) {
    return (el && el._vmarquee) || null;
  }

  const vMarquee = { version: VERSION, init, getInstance, VMarquee };
